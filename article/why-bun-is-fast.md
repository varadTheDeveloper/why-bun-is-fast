*Methodology: every benchmark figure below comes from six controlled experiments (H1–H6) we designed, ran, and independently audited ourselves, on a shared 2-vCPU cloud sandbox using release builds of Bun, Node, and Deno — not dedicated hardware. That limitation applies to every number in this piece and is stated once, here, rather than re-qualified in every sentence. Every source-code claim below links to the exact file (and, where practical, line range) in [Bun's public GitHub repository](https://github.com/oven-sh/bun), read at commit [`8326d1b`](https://github.com/oven-sh/bun/blob/8326d1bd39a96f1f298c3de195aad15972d4f3b4/). Every benchmark claim links to its originating experiment folder in this repository (`experiments/h1-*` through `experiments/h6-*`), where the full methodology, raw data, and analysis scripts are available for independent verification. Every claim about someone else's benchmark or Bun's own marketing is linked and clearly marked as external — never presented as something we measured.*
...
*([H1](../experiments/h1-native-call-boundary/) — native call boundary experiment, 10 runs × 10,000,000 timed calls per combo.)*
...
*([H2](../experiments/h2-fetch-thread-hop/) — fetch thread-hop experiment; 10 runs/runtime, 100 timed cold samples and 100,000 timed keep-alive samples per runtime.)*
...
*([H5](../experiments/h5-buffer-pool/) — buffer allocation experiment, 10 runs × 1,000,000 allocations per size/runtime combo.)*
...
*([H3](../experiments/h3-coldstart-page-faults/) — cold-start page-fault experiment, 30 fresh cold starts per runtime, page cache dropped before each.)*
...
*([H4](../experiments/h4-http-sync-async/) — sync/async handler experiment, 10 runs per combo, 4 combos, 40 runs total.)*
