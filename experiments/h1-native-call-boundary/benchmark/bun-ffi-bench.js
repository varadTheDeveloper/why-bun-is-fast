// H1 bun:ffi benchmark — Bun's own normal/documented native binding
// mechanism (bun:ffi, dlopen-based), calling libnative.so's
// native_increment_i32. This is the PRIMARY Bun-side measurement (as
// distinct from the N-API compat-layer measurement in napi-bench.js,
// which is a supplementary same-binary control — see README.md).
//
// Usage: bun bun-ffi-bench.js <control|test> <iterations>
// Same chunked-timing / stability-check-warmup design as napi-bench.js —
// see that file's header comment for the full methodology rationale.
...
if (!mode || !iterations) {
  console.error("usage: bun-ffi-bench.js <control|test> <iterations>");
  process.exit(1);
}

let native_increment_i32 = null;
if (mode === "test") {
  const { symbols } = dlopen("./libnative.so", {
    native_increment_i32: { args: [FFIType.i32], returns: FFIType.i32 },
  });
  native_increment_i32 = symbols.native_increment_i32;
}
...
function runChunk(n) {
  ...
  if (mode === "control") {
    for (let i = 0; i < n; i++) {
      sink = (i & 0x7fffffff) + 1 | 0;
    }
  } else {
    for (let i = 0; i < n; i++) {
      sink = native_increment_i32(i & 0x7fffffff);
    }
  }
