...
![Bun request lifecycle: socket → uWebSockets → Bun's request object (pooled via HiveArray) → JS handler → sync/async fork (headers+URL cloned on suspension) → response written back (corked writes) → socket](../figures/request-lifecycle.png)
...
![H1 native call boundary overhead by binding path: Bun bun:ffi 14.538ns, Node N-API 22.105ns, Bun N-API compatibility layer on the same binary 81.901ns, Deno Fast API 4.278ns, Deno ordinary path 23.984ns](../figures/h1-binding-path.png)
...
*([H1](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h1-native-call-boundary) — native call boundary experiment, 10 runs × 10,000,000 timed calls per combo.)*
...
*([H2](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h2-fetch-thread-hop) — fetch thread-hop experiment; 10 runs/runtime, 100 timed cold samples and 100,000 timed keep-alive samples per runtime.)*
...
![H5 Buffer allocation throughput vs. size, Bun and Node, log-log scale, showing the winner flip at the 4,096-byte pooling threshold](../figures/h5-buffer-threshold.png)
...
*([H5](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h5-buffer-pool) — buffer allocation experiment, 10 runs × 1,000,000 allocations per size/runtime combo.)*
...
![H3 page faults vs startup time: Bun has fewest total page faults but startup time is statistically tied with Deno](../figures/h3-page-faults.png)
...
*([H3](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h3-coldstart-page-faults) — cold-start page-fault experiment, 30 fresh cold starts per runtime, page cache dropped before each.)*
...
*([H4](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h4-http-sync-async) — sync/async handler experiment, 10 runs per combo, 4 combos, 40 runs total.)*
...
![H6 workload ranking inversion: Deno-Bun-Node on plaintext fully inverts to Bun-Node-Deno on the database-backed workload](../figures/h6-ranking-inversion.png)
...
*([H6](https://github.com/varadTheDeveloper/why-bun-is-fast/tree/main/experiments/h6-realistic-io-convergence) — realistic I/O convergence experiment; concurrency 20, 10-second timed runs.)*
...
![Summary diagram: six experiments, six different variables that flip which runtime wins](../figures/performance-is-not-one-thing.png)
